...
The site went from React islands with hydration directives and inline scripts to a fully static [Astro](https://astro.build/) site with no JavaScript on the client at all. What replaced it was leaner: pure CSS animations (my preference these days) and Astro's view transitions, built on the browser's native [View Transitions API](https://developer.mozilla.org/en-US/docs/Web/API/View_Transition_API) for a blur-fade between pages, adding subtle polish, with no library needed. I experimented with staggered CSS animations on page elements, but cut those too due to jank in iOS Safari, and a sense that they were ultimately still a distraction from the content.
